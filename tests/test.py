...
def gen_data(length):
    ...
    '''
    X = np.random.rand(length, 2).astype(theano.config.floatX)
    y = np.zeros((length, 1)).astype(theano.config.floatX)
    # Compute y[n] = X[n - 1] - *X[n - 2] + noise
    delayed_X = (X[1:-1, 0] - X[:-2, 1] +
                 .01*np.random.randn(length - 2).astype(theano.config.floatX))
    # Store as target
...
l_recurrent = rnntools.LSTMLayer(l_in, num_units=n_hidden)
